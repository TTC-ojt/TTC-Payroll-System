...
namespace TTC_Payroll_System.Forms
{
    public partial class PayrollSummary1 : Form
    {
        ...
        private void dgvPayroll1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            decimal monthly_rate = Convert.ToDecimal(dgvPayroll1.Rows[e.RowIndex].Cells["dgcMonthlyRate"].Value);
            decimal half_rate = Convert.ToDecimal(dgvPayroll1.Rows[e.RowIndex].Cells["dgcHalfRate"].Value);
            ...
            decimal net_pay = Convert.ToDecimal(dgvPayroll1.Rows[e.RowIndex].Cells["dgcNetPay"].Value);

            decimal deductions = sss_loan + pagibig_regular + pagibig_calamity + leaves + withtax;

            if (column == dgcMonthlyRate.Name)
            {
                half_rate = monthly_rate / 2;
                net_pay = half_rate - deductions;
            }
            if (column == dgcHalfRate.Name)
            {
